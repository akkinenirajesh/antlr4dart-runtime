...
class BitSet {
  ...
  bool get(int bitIndex) {
    if (bitIndex < 0) throw new RangeError("bitIndex < 0: $bitIndex");
    return _word & _withBit(bitIndex) > BigInt.zero;
  }

  BigInt _withBit(int bitIndex) {
    return BigInt.one << bitIndex;
  }
  ...
  ///
  ///      for (int i = bs.findNext(0, true); i >= 0; i = bs.nextSetBit(i+1)) {
  ...
  int findNext(int index, bool val) {
    return val?nextSetBit(index):nextClearBit(index);
  }
